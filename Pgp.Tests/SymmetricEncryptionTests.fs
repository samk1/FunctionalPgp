...
module SymmetricEncryptionTests =
    ...
    let ``Decrypt first byte using CryptoStream`` () =
        ...
        let key = Array.zeroCreate 16
        let decryptStream = Pgp.SymmetricEncryption.decrypt Constants.SymmetricKeyAlgorithms.Aes128 key memoryStream
    ...
    let ``Assert encryption is correct`` (data: byte[]) =
        ...
        let key = Array.zeroCreate 16
        let encryptStream = Pgp.SymmetricEncryption.encrypt Constants.SymmetricKeyAlgorithms.Aes128 key memoryStream
